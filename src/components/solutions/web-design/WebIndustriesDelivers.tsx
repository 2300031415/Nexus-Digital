...
import { FiBriefcase, FiHeart, FiHome, FiMaximize, FiSearch, FiShoppingCart, FiStar, FiTool, FiTrendingUp, FiUsers, FiZap } from "react-icons/fi";
...
const delivers = [
  [FiTrendingUp, "More Leads & Enquiries"],
  [FiUsers, "Better User Experience"],
  [FiStar, "Stronger Brand Credibility"],
  [FiSearch, "Higher Google Rankings"],
  [FiZap, "Faster Load Times"],
  [FiMaximize, "Scalable & Flexible"],
] as const;

export default function WebIndustriesDelivers() {
  return (
    <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="bg-nexus-light-bg py-16">
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div>
          <p className="text-nexus-blue text-xs font-semibold uppercase tracking-[0.2em] mb-4">WEB DESIGN ACROSS INDUSTRIES</p>
          ...
          <p className="text-gray-500 text-sm mt-2">Every industry. Every business. We build websites that work.</p>
        </div>
        <div>
          <p className="text-nexus-blue text-xs font-semibold uppercase tracking-[0.2em] mb-4">WHAT GREAT WEB DESIGN DELIVERS</p>
